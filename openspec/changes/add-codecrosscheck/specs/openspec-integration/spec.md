...
The `@codecrosscheck` chat participant SHALL register slash commands `openspec-init`, `openspec-new`, `openspec-implement`, AND `openspec-archive`, mapping respectively to scaffolding, change creation, full-pipeline execution with OpenSpec frame, AND archival.

#### Scenario: Implement command runs full pipeline

- **WHEN** the user sends `@codecrosscheck /openspec-implement add-core-engine`
- **THEN** the participant runs PLAN→CODE→EXECUTE with `--openspec` semantics
