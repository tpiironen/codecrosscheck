...
- [ ] E4. Manual smoke test in EDH. **Still open** — requires a human running
       the Extension Development Host; not something an automated pass can
       tick. Everything else in this change is verified by
       `test/applyReview.test.ts` and `test/applyEdits.test.ts`.
